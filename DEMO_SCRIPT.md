...
- [ ] A **job ad** open on screen (`job ad.txt`) — the Senior Software Engineer / Band 7 / L2 / London one.
- [ ] Monday.com open in a browser tab, on the **dummy source board** and the **tracking board** (so you can flip to them at the end).
- [ ] Have the command ready to paste: `node main.js 'job ad.txt'`
...
```bash
node main.js 'job ad.txt'          # the live demo — full pipeline, DRY-RUN (writes nothing)
node main.js 'job ad.txt' --post   # the "go live" step — posts the comment + tracking item
```
